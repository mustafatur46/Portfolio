  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 px-5 pt-4 pb-3.5 flex-shrink-0 border-b border-white/[0.06]">
          <div className="min-w-0">
            ...
            <h3 className="text-white font-bold text-[1.05rem] leading-snug">{p.title}</h3>
          </div>
          {p.architecture && (
            <button
        ...
        <div className="relative h-[420px] flex-shrink-0" style={{ background: `${color}06` }}>
          {p.architecture ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-center px-6">
              ...
              <p className="text-[0.85rem] text-[#777] font-semibold">Architektur-Diagramm folgt</p>
              <p className="text-[0.72rem] text-[#555] max-w-[320px]">
                Für dieses Projekt ist (noch) kein detailliertes Diagramm hinterlegt — siehe Overview unten.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-5 flex flex-col gap-5 border-t border-white/[0.06]">
          {/* Overview */}
          <div>
            <p className="text-[0.66rem] font-bold uppercase tracking-[1.5px] text-[#444] mb-2">Overview</p>
            <p className="text-[0.86rem] text-[#aaa] leading-[1.78]">{p.fullDescription}</p>
          </div>

          {/* Key decisions */}
          {p.architecture?.notes && p.architecture.notes.length > 0 && (
            <div>
              <p className="text-[0.66rem] font-bold uppercase tracking-[1.5px] text-[#444] mb-2.5">Key Decisions</p>
          ...
          {/* Tech stack */}
          <div>
            <p className="text-[0.66rem] font-bold uppercase tracking-[1.5px] text-[#444] mb-2.5">Tech Stack</p>
          ...
          {p.links && p.links.length > 0 && (
            <div>
              <p className="text-[0.66rem] font-bold uppercase tracking-[1.5px] text-[#444] mb-2.5">Links</p>
      ...
      {fullscreen && p.architecture && (
        <ArchitectureOverlay project={p} color={color} onClose={() => setFullscreen(false)} />
      )}
    </div>
  );
}
